...
) -> Result<curandGenerator_t, crate::sys::curandStatus_t> {
    let mut out_0: std::mem::MaybeUninit<curandGenerator_t> = std::mem::MaybeUninit::uninit();
...
pub unsafe fn curandGenerateNormal<T: types::CudaAsPtr>(
    ...
    let status =
        unsafe { crate::sys::curandGenerateNormal(generator, outputPtr.as_mut_ptr() as *mut _, n, mean, stddev) };
    if status == crate::sys::curandStatus_t::CURAND_STATUS_SUCCESS {
        Ok(())
    } else {
        Err(status)
...
pub unsafe fn curandGenerateNormalDouble<T: types::CudaAsPtr>(
    ...
    let status =
        unsafe { crate::sys::curandGenerateNormalDouble(generator, outputPtr.as_mut_ptr() as *mut _, n, mean, stddev) };
    if status == crate::sys::curandStatus_t::CURAND_STATUS_SUCCESS {
        Ok(())
    } else {
        Err(status)
...
pub unsafe fn curandGenerateLogNormal<T: types::CudaAsPtr>(
    ...
    let status =
        unsafe { crate::sys::curandGenerateLogNormal(generator, outputPtr.as_mut_ptr() as *mut _, n, mean, stddev) };
    if status == crate::sys::curandStatus_t::CURAND_STATUS_SUCCESS {
        Ok(())
    } else {
        Err(status)
...
pub unsafe fn curandGenerateLogNormalDouble<T: types::CudaAsPtr>(
    ...
    let status = unsafe {
        crate::sys::curandGenerateLogNormalDouble(generator, outputPtr.as_mut_ptr() as *mut _, n, mean, stddev)
    };
    if status == crate::sys::curandStatus_t::CURAND_STATUS_SUCCESS {
        Ok(())
    } else {
...
pub unsafe fn curandCreatePoissonDistribution(
    lambda: f64,
) -> Result<curandDiscreteDistribution_t, crate::sys::curandStatus_t> {
    let mut out_1: std::mem::MaybeUninit<curandDiscreteDistribution_t> = std::mem::MaybeUninit::uninit();
    let status = unsafe { crate::sys::curandCreatePoissonDistribution(lambda, out_1.as_mut_ptr() as *mut _) };
    if status as usize == crate::sys::curandStatus_t::CURAND_STATUS_SUCCESS as usize {
        unsafe { Ok(out_1.assume_init() as curandDiscreteDistribution_t) }
    } else {
        Err(unsafe { std::mem::transmute(status) })
    }
}
...
pub unsafe fn curandGeneratePoissonMethod<T: types::CudaAsPtr>(
    ...
    let status = unsafe {
        crate::sys::curandGeneratePoissonMethod(generator, outputPtr.as_mut_ptr() as *mut _, n, lambda, method)
    };
    if status == crate::sys::curandStatus_t::CURAND_STATUS_SUCCESS {
        Ok(())
    } else {
...
pub unsafe fn curandGenerateBinomial<T: types::CudaAsPtr>(
    ...
    let status =
        unsafe { crate::sys::curandGenerateBinomial(generator, outputPtr.as_mut_ptr() as *mut _, num, n as _, p) };
    if status == crate::sys::curandStatus_t::CURAND_STATUS_SUCCESS {
        Ok(())
    } else {
        Err(status)
...
pub unsafe fn curandGenerateBinomialMethod<T: types::CudaAsPtr>(
    ...
    let status = unsafe {
        crate::sys::curandGenerateBinomialMethod(generator, outputPtr.as_mut_ptr() as *mut _, num, n as _, p, method)
    };
    if status == crate::sys::curandStatus_t::CURAND_STATUS_SUCCESS {
        Ok(())
    } else {
